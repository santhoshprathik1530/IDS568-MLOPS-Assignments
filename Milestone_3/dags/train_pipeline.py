...
def on_failure_callback(context):
    ...
    print(f"[FAILURE] task={ti.task_id if ti else 'unknown'}")
...
def train_model(**context):
    ...
    run = client.search_runs([e.experiment_id], order_by=["attributes.start_time DESC"], max_results=1)[0]
    return {"run_id": run.info.run_id, "model_name": model_name, "tracking_uri": tracking_uri}
...
def register_model(**context):
    ...
    client.update_model_version(name=model_name, version=str(reg.version), description=f"Registered by Airflow from run {run_id}")
